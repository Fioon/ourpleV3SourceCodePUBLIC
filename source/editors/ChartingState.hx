...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['--OURPLE--',''],
		['PlayVideoSprite','nameofvideo','camera'],
		['tweenZoom', 'value 1: zoom,speed,stay at zoom?\nvalue 2: ease'],
		['BGswap','only works with stage class'],
		['updateHUD','use if u swapped pixelstage'],
		['TweenObjectAlpha',''],
		['TweenNotesAlpha',''],
		['camShakeTween'],
		['camFlash', 'value1: color,how long it lasts\nvalue2: type what camera to flash\n game or nothing for only game\n hud to flash the hud\n other to flash everything'],
		['snapZoom','value1: set cam zoom level'],
		['setNotePosition','value1: pick where notes will be\nEMPTY for default\nMIDDLE for middlescroll\nSWAP to switch places with opponent'],
		['toggleCamVisible',' using this event will toggle if the camera is visible'],
		['--SHADERS--',''],
		['mosaic',''],
		['dotMatrix',''],
		['dismantledShader',''],
	...
	**/
	...
	var check_PixelNoteSect:FlxUICheckBox;
	...
	function addSectionUI():Void
	{
		...
		check_PixelNoteSect = new FlxUICheckBox(check_mustHitSection.x + 120, check_mustHitSection.y, null, null, "Pixel Notes", 100);
		check_PixelNoteSect.name = 'chec_PixelNoteSect';
		check_PixelNoteSect.checked = _song.notes[curSec].pixelNoteSection;
		...
		tab_group_section.add(check_PixelNoteSect);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			{
				case 'Pixel Notes':
					_song.notes[curSec].pixelNoteSection = check.checked;
	...
	override function update(elapsed:Float)
	{
		...
		'\nTime: ' + Std.string(FlxStringUtil.formatTime(Math.floor(Conductor.songPosition / 1000))) + ' / ${FlxStringUtil.formatTime(Math.floor(FlxG.sound.music.length)/1000,false)}' +
		"\nSection: " + curSec +
	...
	*/
	...
	function updateSectionUI():Void
	{
		...
		check_PixelNoteSect.checked = sec.pixelNoteSection;
